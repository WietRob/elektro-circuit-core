...
test.describe('Folgeschaltung - Generischer Runtime-Test', () => {
  ...
  test('Transition: S1.pressed → k1_only', async ({ page }) => {
    ...
    const p1State = await page.locator('[data-component="P1"]').first().getAttribute('data-projected-state');
  ...
  test('Transition: S2.pressed → k1_and_k2', async ({ page }) => {
    ...
    const p2State = await page.locator('[data-component="P2"]').first().getAttribute('data-projected-state');
